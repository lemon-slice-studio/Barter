...
public class TradeStationLockMessage implements INormalMessage
{
    private final BlockPos pos;

    public TradeStationLockMessage(BlockPos pos)
    {
        this.pos = pos;
    }

    public TradeStationLockMessage(PacketBuffer buf)
    {
        ...
        if (ctx.getDirection() == NetworkDirection.PLAY_TO_SERVER)
            ctx.enqueueWork(() ->
            {
                if (world.isAreaLoaded(pos, 1))
                {
                    ...
                    {
                        ((TradeStationBlockTileEntity) te).lockOrUnlock();
                    }
                }
            });
    }
