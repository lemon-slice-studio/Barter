...
public class TradeStationTextChangeMessage implements INormalMessage
{
    private final String text;
    private final BlockPos pos;

    public TradeStationTextChangeMessage(String text, BlockPos pos)
    {
        this.text = text;
        this.pos = pos;
    }

    public TradeStationTextChangeMessage(PacketBuffer buf)
    {
    ...
    {
        packetBuffer.writeString(text, 32767);
        ...
        if (ctx.getDirection() == NetworkDirection.PLAY_TO_SERVER)
            ctx.enqueueWork(() ->
            {
                if (world.isAreaLoaded(pos, 1))
                {
                    ...
                    {
                        ((TradeStationBlockTileEntity) te).setInput(text);
                        te.markDirty();
                    }
                }
            });
